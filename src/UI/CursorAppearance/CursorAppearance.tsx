...
const CursorAppearance: FC<ICursor> = ({x1, y1, x2, y2, Display, Position}) => {
	return (
		<>
			<article style={{ translate: `${x1}px ${y1}px`, display: Display, position: Position as Position }} className={styles.CenterDot}></article>
			<article style={{ translate: `${x2}px ${y2}px`,  display: Display, position: Position as Position }} className={styles.Dot}></article>
		</>
	)
}
